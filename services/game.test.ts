...
import { MISSIONS, activeMission, missionConditionsMet } from '../data/missions';
import { nextBehaviorVariant } from '../data/behaviors';
...
describe('catalog and playable progression', () => {
  ...
  it('uses local calendar days, all four boundaries, and stable daily forecasts', () => {
    ...
    for (const [hour, time] of [[4, TimeOfDay.Night], [5, TimeOfDay.Morning], [10, TimeOfDay.Day], [16, TimeOfDay.Sunset], [19, TimeOfDay.Night]] as const) {
      expect(currentTimeOfDay(new Date(2026, 8, 19, hour))).toBe(time);
    }
...
describe('investigations and repeat observation', () => {
  it('advances investigation missions only when the clue conditions are satisfied', () => {
    const first = MISSIONS[0];
    expect(activeMission([])?.id).toBe(first.id);
    expect(missionConditionsMet(first, first.targetCreatureId, first.areaId, first.time, [])).toBe(true);
    expect(missionConditionsMet(first, first.targetCreatureId, 'house', first.time, [])).toBe(false);

    const itemMission = MISSIONS.find(m => m.requiredItemId)!;
    expect(missionConditionsMet(itemMission, itemMission.targetCreatureId, itemMission.areaId, itemMission.time, [])).toBe(false);
    expect(missionConditionsMet(itemMission, itemMission.targetCreatureId, itemMission.areaId, itemMission.time, [itemMission.requiredItemId!])).toBe(true);

    let p = reduce(emptyProgress(), { type: 'missionComplete', id: first.id });
    expect(p.completedMissionIds).toEqual([first.id]);
    p = reduce(p, { type: 'missionSeen', id: first.id });
    expect(p.seenMissionPostscriptIds).toEqual([first.id]);
    expect(activeMission(p.completedMissionIds)?.id).toBe(MISSIONS[1].id);
  });

  it('adds a new ecology record on supported repeat observations without changing species completion', () => {
    let p = reduce(emptyProgress(), { type: 'capture', id: '001' });
    const variant = nextBehaviorVariant('001', []);
    expect(variant).not.toBeNull();
    p = reduce(p, { type: 'capture', id: '001', behaviorVariantId: variant!.id });
    expect(p.discoveredIds).toEqual(['001']);
    expect(p.observations['001']).toEqual([variant!.id]);
    expect(p.inventory).toEqual(['item_nut']);
    const second = nextBehaviorVariant('001', p.observations['001']);
    expect(second?.id).not.toBe(variant!.id);
  });
});

describe('save, inventory and buddy integrity', () => {
  ...
  it('limits bond-gaining pet interactions to three per day and records memories', () => {
    let p = reduce(emptyProgress(), { type: 'capture', id: '001' });
    p = reduce(p, { type: 'buddy', id: '001' });
    for (let i = 0; i < 10; i++) p = reduce(p, { type: 'pet', date: '2026-09-19' });
    expect(p.bonds['001']).toBe(6);
    expect(p.petCount).toBe(3);
    expect(p.buddyMemories['001']).toContain('first-pet');
    p = reduce(p, { type: 'pet', date: '2026-09-20' });
    expect(p.bonds['001']).toBe(8);
    expect(p.petCount).toBe(1);
  });

  it('preserves per-species bonds, exploration memories and caps bond at 100', () => {
    let p = reduce(emptyProgress(), { type: 'capture', id: '001' });
    p = reduce(p, { type: 'capture', id: '002' });
    p = reduce(p, { type: 'buddy', id: '001' });
    p = reduce(p, { type: 'expedition', areaId: 'water' });
    p = reduce(p, { type: 'capture', id: '002' });
    p = reduce(p, { type: 'item', id: 'item_candy' });
    p = reduce(p, { type: 'feed', id: 'item_candy' });
    expect(p.bonds['001']).toBe(20);
    expect(p.buddyMemories['001']).toEqual(expect.arrayContaining(['first-expedition', 'area:water', 'capture-together', 'first-snack', 'bond:20']));

    p = reduce(p, { type: 'buddy', id: '002' });
    p = reduce(p, { type: 'pet', date: '2026-09-19' });
    p = reduce(p, { type: 'buddy', id: '001' });
    expect(p.bonds['002']).toBe(2);
    expect(p.bonds['001']).toBe(20);
    ...
    expect(p.bonds['001']).toBe(100);
    expect(p.buddyMemories['001']).toContain('bond:100');
  ...
  it('roundtrips a completed v2 save and preserves new progress fields', () => {
    ...
    p = reduce(p, { type: 'pet', date: '2026-09-19' });
    p = reduce(p, { type: 'expedition', areaId: 'house' });
    p = reduce(p, { type: 'missionComplete', id: MISSIONS[0].id });
    p = reduce(p, { type: 'read', milestone: 33 });
    expect(parseProgress(JSON.stringify(p))).toEqual(p);
  });

  it('migrates valid v1 saves without losing the original game progress', () => {
    const v1 = {
      version: 1,
      userName: '旧データ',
      onboarded: true,
      ...
      bonds: { '001': 42 },
      readMilestones: [],
      lastRewardDate: '2026-09-18',
      captures: 3
    };
    const migrated = parseProgress(JSON.stringify(v1));
    expect(migrated.version).toBe(2);
    expect(migrated.userName).toBe('旧データ');
    expect(migrated.discoveredIds).toEqual(['001']);
    expect(migrated.bonds['001']).toBe(42);
  ...
  it('rejects malformed or unsupported saves and sanitizes unknown catalog data', () => {
    for (const raw of ['null', '{}', '{', '[]', JSON.stringify({ ...emptyProgress(), version: 3 }), JSON.stringify({ ...emptyProgress(), bonds: [] }), JSON.stringify({ ...emptyProgress(), captures: -1 })]) {
      expect(() => parseProgress(raw)).toThrow();
    }
    const clean = parseProgress(JSON.stringify({
      ...emptyProgress(),
      discoveredIds: ['001', '001', 'debug_0'],
      favorites: ['002'],
      inventory: ['fake'],
      buddyId: '002',
      bonds: { '001': 1000 },
      readMilestones: [33],
      completedMissionIds: ['fake'],
      observations: { '001': ['fake'] },
      buddyMemories: { '001': ['fake'] }
    }));
    expect(clean.discoveredIds).toEqual(['001']);
    expect(clean.bonds).toEqual({ '001': 100 });
    expect(clean.favorites).toEqual([]);
    expect(clean.buddyId).toBeNull();
    expect(clean.inventory).toEqual([]);
    expect(clean.readMilestones).toEqual([]);
    expect(clean.completedMissionIds).toEqual([]);
    expect(clean.observations).toEqual({});
    expect(clean.buddyMemories).toEqual({});
  });
});
